    ...
    <div class="w-full max-w-md">
        ...
        <form method="POST" action="{{ route('login') }}" class="space-y-4 rounded-xl bg-slate-800 p-8 shadow-xl ring-1 ring-slate-700">
            ...
            @if (session('status_error'))
                <p class="text-sm text-red-400">{{ session('status_error') }}</p>
            @endif
            ...
            <a href="{{ route('google.redirect') }}"
               class="flex w-full items-center justify-center gap-3 rounded-md bg-white px-4 py-2 text-sm font-medium text-slate-700 shadow-sm hover:bg-slate-100 focus:outline-none focus:ring-2 focus:ring-slate-400 focus:ring-offset-2">
